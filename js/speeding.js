function renderSpeedCharts(data) {
    ...
    generateSpAgeDonut(spData); 
    generateSpJurisdictionBar(spData); 
    generateSpLineChart(spData);
}
...
function generateSpAgeDonut(data) {
    ...
    const slices = svg.selectAll("path").data(pie(cD)).enter().append("path")
        .attr("fill", d => color(d.data.label)).attr("stroke", "#ffffff").style("stroke-width", "1px")
        .style("transition", "opacity 0.2s, stroke-width 0.2s").attr("d", arcZero)
        .style("cursor", "pointer")
        .each(function(d) { this._current = { startAngle: d.startAngle, endAngle: d.startAngle }; });
    
    function highlight(dL, dV, isClick = false) {
        // Prevent generic hover events from resetting an actively selected path slice
    ...
    // Attach interaction events cleanly right away
